...
//					};
          ...
          insert_vosl += '</div>';
					...
					insert_vosl += '<div class="channel_profile_icon"><img loading="lazy" src="' + csvList_vosl[i][26] + '" alt="' + csvList_vosl[i][1] + '" class="animated fadeIn"></div>';
          ...
          insert_vosl += '<div class="row row-cols-1 row-cols-sm-2 animated fadeIn">';
          insert_vosl += '<div class="col">';
...
//					insert_vosl += '<iframe src="https://www.youtube.com/embed/' + youtubeid + '" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>';
          ...
          insert_vosl += '<div class="col">';
					 ...
					 if(params.length>1){
							var url = decodeURI(location.search)
							if(url.indexOf('lang=en') !== -1){
								if(csvList_vosl[i][28]==""){}else{
								insert_vosl += '<div class="mb-1" style="color:#7657ea;"><img src="Images/google-gemini-icon.svg" alt="gemini icon" style=" margin-top:-5px;width:15px;" class="animated rotateIn" loading="lazy"> AI Review</div>';
								insert_vosl += '<div class="mb-1 p-2 border rounded animated fadeIn">' + csvList_vosl[i][28] + '</div>';
								insert_vosl += '<div class="mb-2 text-end text-muted" style="font-size:11px;">※This review was generated by AI (Google Gemini).</div>';
								};
							} else if(url.indexOf('lang=ko') !== -1){
								if(csvList_vosl[i][28]==""){}else{
								insert_vosl += '<div class="mb-1" style="color:#7657ea;"><img src="Images/google-gemini-icon.svg" alt="gemini icon" style=" margin-top:-5px;width:15px;" class="animated rotateIn" loading="lazy"> AI 리뷰</div>';
								insert_vosl += '<div class="mb-1 p-2 border rounded animated fadeIn">' + csvList_vosl[i][28] + '</div>';
								insert_vosl += '<div class="mb-2 text-end text-muted" style="font-size:11px;">※이 감상은 AI(Google Gemini)에 의해 생성되었습니다.</div>';
								};
							} else if(url.indexOf('lang=es') !== -1){
								if(csvList_vosl[i][28]==""){}else{
								insert_vosl += '<div class="mb-1" style="color:#7657ea;"><img src="Images/google-gemini-icon.svg" alt="gemini icon" style=" margin-top:-5px;width:15px;" class="animated rotateIn" loading="lazy"> Revisión de IA</div>';
								insert_vosl += '<div class="mb-1 p-2 border rounded animated fadeIn">' + csvList_vosl[i][28] + '</div>';
								insert_vosl += '<div class="mb-2 text-end text-muted" style="font-size:11px;">※Esta reseña ha sido generada por IA (Google Gemini).</div>';
								};
							}else if(url.indexOf('lang=zh-TW') !== -1){
								if(csvList_vosl[i][28]==""){}else{
								insert_vosl += '<div class="mb-1" style="color:#7657ea;"><img src="Images/google-gemini-icon.svg" alt="gemini icon" style=" margin-top:-5px;width:15px;" class="animated rotateIn" loading="lazy"> AI 評論</div>';
								insert_vosl += '<div class="mb-1 p-2 border rounded animated fadeIn">' + csvList_vosl[i][28] + '</div>';
								insert_vosl += '<div class="mb-2 text-end text-muted" style="font-size:11px;">※此感想由AI（Google Gemini）生成。</div>';
								};
							}else{
							if(csvList_vosl[i][27]==""){}else{
								insert_vosl += '<div class="mb-1" style="color:#7657ea;"><img src="Images/google-gemini-icon.svg" alt="gemini icon" style=" margin-top:-5px;width:15px;" class="animated rotateIn" loading="lazy"> AI レビュー</div>';
								insert_vosl += '<div class="mb-1 p-2 border rounded animated fadeIn">' + csvList_vosl[i][27] + '</div>';
								insert_vosl += '<div class="mb-2 text-end text-muted" style="font-size:11px;">※この感想はAI（Google Gemini）により生成されています。</div>';
							};
							}
						} else{
							if(csvList_vosl[i][27]==""){}else{
