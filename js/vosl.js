          ...
          insert_vosl += '<div class="col">';
					 ...
					 if(params.length>1){
							...
							if(url.indexOf('lang=en') !== -1){
								...
								insert_vosl += '<div class="mb-1" style="color:#7657ea;">⭐️ AI Review</div>';
								insert_vosl += '<div class="mb-1 p-2 border rounded">' + csvList_vosl[i][28] + '</div>';
								insert_vosl += '<div class="mb-2 text-end text-muted" style="font-size:11px;">※This review was generated by AI (Google Gemini).</div>';
								};
							} else if(url.indexOf('lang=ko') !== -1){
								if(csvList_vosl[i][28]==""){}else{
								insert_vosl += '<div class="mb-1" style="color:#7657ea;">⭐️ AI 리뷰</div>';
								insert_vosl += '<div class="mb-1 p-2 border rounded">' + csvList_vosl[i][28] + '</div>';
								insert_vosl += '<div class="mb-2 text-end text-muted" style="font-size:11px;">※이 감상은 AI(Google Gemini)에 의해 생성되었습니다.</div>';
								};
							} else if(url.indexOf('lang=es') !== -1){
								if(csvList_vosl[i][28]==""){}else{
								insert_vosl += '<div class="mb-1" style="color:#7657ea;">⭐️ Revisión de IA</div>';
								insert_vosl += '<div class="mb-1 p-2 border rounded">' + csvList_vosl[i][28] + '</div>';
								...
								if(csvList_vosl[i][28]==""){}else{
								insert_vosl += '<div class="mb-1" style="color:#7657ea;">⭐️ AI 評論</div>';
								insert_vosl += '<div class="mb-1 p-2 border rounded">' + csvList_vosl[i][28] + '</div>';
								insert_vosl += '<div class="mb-2 text-end text-muted" style="font-size:11px;">※此感想由AI（Google Gemini）生成。</div>';
								};
							}else{
							if(csvList_vosl[i][27]==""){}else{
								insert_vosl += '<div class="mb-1" style="color:#7657ea;">⭐️ AI レビュー</div>';
								insert_vosl += '<div class="mb-1 p-2 border rounded">' + csvList_vosl[i][27] + '</div>';
